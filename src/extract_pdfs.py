    ...
    for pdf_file in pdf_files:
        ...
        for idx, sec in enumerate(sections, start=1):
            ...
            if embedding is None:
                print(f"Skipping upsert for section {idx} of {doc_name}: embedding failed.")
                continue

            # Use a plain UUID for the document ID to avoid illegal characters
            item_id = str(uuid.uuid4())

            # Prepare item for upsert into Cosmos DB
            item = {
                "id": item_id,
            ...
            try:
                ...
                print(f"Upserted: {doc_name} - Section {idx} (ID: {item_id})")
            except Exception as e:
                print(f"Failed to upsert {doc_name} - Section {idx}: {e}")
